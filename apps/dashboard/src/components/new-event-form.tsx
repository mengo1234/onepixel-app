...
export function NewEventForm({ venues, paymentId, participantLimit }: { venues: Venue[]; paymentId: string; participantLimit: number }) {
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(`/events/${payload.id}/layout`);
      router.refresh();
    } catch (caught) {
        ...
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: .22 }}>
            {step === 0 && <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_320px]">
              <section><p className="font-mono text-[9px] uppercase tracking-[.2em] text-[#d1e66a]">EVENTO E LUOGO FISICO</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.045em]">Scegli cosa accade e dove.</h2><p className="mt-3 max-w-2xl text-xs leading-5 text-[#7f8885]">Il tipo descrive l'evento. La struttura resta un luogo riutilizzabile; palco, platea e transenne verranno preparati nel passo successivo.</p><div className="mt-7 grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2"><span className="editor-label">Nome evento</span><input value={title} onChange={(event) => setTitle(event.target.value)} className={inputClass} placeholder="Finale Luce" autoFocus /></label><label><span className="editor-label">Tipo di evento</span><select value={kind} onChange={(event) => { const next = event.target.value as typeof kind; setKind(next); if (next === "parade") setMethods((current) => current.includes("mobile_radius") ? current : [...current, "mobile_radius"]); }} className={inputClass}>{kinds.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span className="editor-label">Struttura fisica</span><select value={venueId} onChange={(event) => setVenueId(event.target.value)} className={inputClass}>{venues.map((venue) => <option key={venue.id} value={venue.id}>{venue.name}</option>)}</select></label><label className="sm:col-span-2"><span className="editor-label">Pianta di partenza</span><select value={layoutId} onChange={(event) => setLayoutIds((current) => ({ ...current, [venueId]: event.target.value }))} className={inputClass}>{selectedVenue?.layouts.map((layout) => <option key={layout.id} value={layout.id}>{layout.name} · {layout.capacity.toLocaleString("it-IT")} posti</option>)}</select><span className="mt-1.5 block text-[9px] leading-4 text-[#68716f]">onePixel ne crea una copia congelata per questo evento: l'originale non verrà modificato.</span></label><label className="sm:col-span-2"><span className="editor-label">Descrizione</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={4} className="w-full rounded-xl border border-white/10 bg-[#0b0e0f] p-3.5 text-xs leading-5 text-white outline-none focus:border-[#d1e66a]/55" placeholder="Racconta cosa vedranno i partecipanti…" /></label></div></section>
            ...
            {step === 3 && <div className="mx-auto max-w-3xl"><p className="font-mono text-[9px] uppercase tracking-[.2em] text-[#d1e66a]">RIEPILOGO</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.045em]">Pronto per preparare lo spazio.</h2><div className="mt-7 divide-y divide-white/8 rounded-[28px] border border-white/10 bg-[#0b0e0f] px-5"><Summary label="Evento" value={`${title} · ${kinds.find(([value]) => value === kind)?.[1]}`} /><Summary label="Struttura" value={`${selectedVenue?.name} · ${selectedLayout?.name}`} /><Summary label="Luogo" value={locationName} /><Summary label="Ingresso" value={methods.map((method) => method === "qr" ? "QR" : method === "fixed_geofence" ? "GPS fisso" : "Capofila mobile").join(" + ")} /><Summary label="Pubblico" value={`fino a ${participantLimit.toLocaleString("it-IT")} partecipanti`} /><Summary label="Controlli" value={`${audioAllowed ? "audio" : "no audio"} · ${torchAllowed ? "torcia" : "no torcia"}`} /></div><p className="mt-5 rounded-2xl border border-[#d1e66a]/20 bg-[#d1e66a]/6 p-4 text-xs leading-5 text-[#aeb69d]">Dopo la creazione si apre l'allestimento: aggiungerai palco, passerella, platea e zone chiuse sulla copia dell'evento. Poi passerai a effetti, QR e regia.</p></div>}
          </motion.div>
        </AnimatePresence>
      </div>
